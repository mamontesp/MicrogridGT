...
def pv_utility_fn(pv, wt, de, bt, ld, dt):
	return (pv_unit_electric_price-pv_unit_maintenance_cost)*pv*dt \
...
def wt_utility_fn(pv, wt, de, bt, ld, dt):
	return (wt_unit_electric_price-wt_unit_maintenance_cost)*wt*dt \
	- alpha * np.power(penalty_fn((pv,wt,de,bt,ld),2))

##Utility function of DE taking into account power balance
def de_utility_fn(pv, wt, de, bt, ld, dt):
	return (de_unit_electric_price-de_unit_maintenance_cost)*de*dt - de_oil_price * de_rate_oil_consumption * dt\
		- alpha * np.power(penalty_fn((pv,wt,de,bt,ld),2))
...
def bt_utility_fn(pv, wt, de, bt, ld, dt):
...
def ld_utility_fn(pv, wt, de, bt, ld, nom_ld, dt):
	return -1*((1-ld_weight_satisfaction)*ld_unit_electric_price*ld + ld_weight_satisfaction*ld_satifaction_fn(ld, nom_ld))

## Satisfaction function of load consumption
def ld_satifaction_fn(ld, nom_ld):
	return nom_ld*ld_beta*(np.power((ld/nom_ld),ld_ak) -1)
